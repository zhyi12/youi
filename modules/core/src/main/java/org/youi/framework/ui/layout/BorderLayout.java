...
public class BorderLayout extends AbstractLayout{
	...
	public String getStartHtml(PageContext pageContext){
		...
		//script
		htmls.append("<script>$(function(){$('body',document).borderLayout({")
			.append("});")
			.append("});</script>");
		...
		if(bodyPanel!=null){
			htmls.append("<td valign=\"top\"><div class=\"layout-panel panel-").append(bodyPanelKey).append("\">");
	...
	private String loadPanel(PageContext pageContext,String panelKey) {
		...
		if(panels.containsKey(panelKey)){
			...
			htmls.append("<div style=\"width:"+panel.getWidth()+"px;\" class=\"layout-panel panel-").append(panelKey).append("\">");
			if(jspFile!=null){
				try {
